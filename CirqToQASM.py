...
from cirq import Circuit, QasmArgs, qasm

@dataclass
class CirqConversionStats:
    """Statistics about the converted Cirq circuit."""
    n_qubits: Optional[int]
    depth: Optional[int]
    n_moments: Optional[int]
...
class CirqToQASM3Converter:
    ...
    def _analyze_cirq_circuit(self, circuit: 'Circuit') -> CirqConversionStats:
        """
        ...
        Returns:
            CirqConversionStats: Circuit analysis statistics
        """
        try:
    ...
    def _convert_to_qasm3(self, circuit: 'Circuit') -> str:
        """
        Convert Cirq Circuit to OpenQASM 3.0 string.
    ...
    def convert(self, cirq_source: str) -> tuple[str, CirqConversionStats]:
        """
        ...
        Returns:
            tuple[str, CirqConversionStats]: OpenQASM 3.0 program and conversion statistics
        ...
        Example:
            ...
            >>> print(f"Circuit has {stats.n_qubits} qubits and {stats.depth} moments")
        """
...
def convert_cirq_to_qasm3(cirq_source: str) -> str:
    """
    ...
    Example:
        ...
        ...     return circuit
        ... '''
        >>> qasm_program = convert_cirq_to_qasm3(source)
        >>> print(qasm_program)
    """
    converter = CirqToQASM3Converter()
    qasm_program, _ = converter.convert(cirq_source)
    return qasm_program


def convert_cirq_to_qasm3_with_stats(cirq_source: str) -> tuple[str, Dict[str, Any]]:
    """
    Convert Cirq source code to OpenQASM 3.0 with detailed statistics.
    ...
    Example:
        >>> qasm_program, stats = convert_cirq_to_qasm3_with_stats(source)
        >>> print(f"Converted circuit with {stats['n_qubits']} qubits")
    """
    converter = CirqToQASM3Converter()
    qasm_program, stats_obj = converter.convert(cirq_source)
    
    # Convert stats object to dictionary for easier use
    stats_dict = {
        'n_qubits': stats_obj.n_qubits,
        'depth': stats_obj.depth,
        'n_moments': stats_obj.n_moments,
        'gate_counts': stats_obj.gate_counts,
        'has_measurements': stats_obj.has_measurements
    }
