...
The simulation module requirements capture the execution behaviour of the QSim backends. The QSim SDK is published on PyPI and operates independently of the QCanvas Web IDE.
\begin{itemize}
...
\begin{itemize}
    ...
    \textit{Confidentiality:} Sensitive API keys shall be encrypted at rest using AES-256. \textit{Integrity:} User passwords shall be hashed using Bcrypt with per-user salts, and all state transitions shall be validated server-side. \textit{Availability:} Role-Based Access Control (RBAC) shall ensure each role can only access permitted resources, limiting the blast radius of any single compromised account.
\end{itemize}
