...
class CirqToQASM3Converter:
    ...
    leveraging Cirq's native OpenQASM 3.0 export functionality.
    ...
    def _add_ast_operation(self, builder: QASM3Builder, operation):
        ...
        if isinstance(operation, GateNode):
            ...
            if gate_name in ['h', 'x', 'y', 'z', 's', 't', 'sx', 'i', 'id']:
                ...
                if operation.parameters:
                    param = builder.format_parameter(operation.parameters[0])
                    builder.apply_gate(gate_name, [], parameters=[param])
                else:
                    builder.add_comment("Global phase gate missing parameter")
            elif gate_name in ['matrix', 'unitary']:
                # Matrix/unitary gates - add as comment since full matrix support would be complex
                builder.add_comment(f"Matrix gate: {gate_name} (not fully supported)")
            elif gate_name.startswith('phased'):
                # Phased gates - simplified handling
                builder.add_comment(f"Phased gate: {gate_name} (simplified)")
            elif gate_name.startswith('controlled'):
                # Controlled gates - simplified handling
                builder.add_comment(f"Controlled gate: {gate_name} (simplified)")
            else:
    ...
    def convert(self, cirq_source: str) -> ConversionResult:
        """
        Convert Cirq source code to OpenQASM 3.0 format using AST-based parsing.
