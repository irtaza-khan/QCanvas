  ...
  return (
    <div className="w-12 bg-editor-sidebar flex flex-col items-center py-2 gap-1.5">
      {items.map(({ id, label, Icon }) =>
        (() => {
          const isActive =
            id === "cirqAssistant" ? !!assistantOpen : active === id;
          return (
            <button
              key={id}
              type="button"
              ...
              className={`relative w-10 h-10 rounded-md flex items-center justify-center transition-all ${
                isActive
                  ? "bg-gray-600/20 text-white shadow-[0_0_18px_rgba(70,234,237,0.14)]"
                  : // : "text-editor-text hover:bg-purple-600/50 hover:text-white"
                    "text-editor-text hover:bg-gray-400/50 hover:text-white"
              }`}
            >
              {isActive && (
